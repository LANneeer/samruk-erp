...
from src.infrastructure.async_unit_of_work import AsyncUnitOfWork, get_uow
...
async def update_user(
    ...
    await bus.handle(
        UpdateUserProfile(
            user_id=user_id, new_username=dto.username, new_locale=dto.locale
        )
    )
    user = await uow.users.get_async(user_id)
    if not user:
...
async def change_password(
    ...
    await bus.handle(
        ChangeUserPassword(
            user_id=user_id, new_password_hash=hash_password(dto.password)
        )
    )
    return None
